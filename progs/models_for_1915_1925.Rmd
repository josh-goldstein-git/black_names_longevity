...
Summary: In this notebook, I calculate the BNI gradient for by race and geography for the birth cohorts of 1915-1925. 
...
```{r}
dt %>% 
  group_by(nickname_master) %>% 
  tally() %>% 
  mutate(freq = n / sum(n))

dt %>% 
  group_by(nickname_source) %>% 
  tally() %>% 
  mutate(freq = n / sum(n))
...
m.bni.black = felm(death_age ~ bni_root_master | key + birth_order + as.factor(byear),
                   subset =
                     nkey_male %in% 2:5 &
                     sex_score_std_strong < 1.3 & 
                     byear %in% 1915:1925 &
...
m.bni.white = felm(death_age ~ bni_root_master | key + birth_order + as.factor(byear),
                   subset =
                     nkey_male %in% 2:5 &
                     sex_score_std_strong < 1.3 & 
                     byear %in% 1915:1925 &
...
m.bni.south = felm(death_age ~ bni_root_master | key + birth_order + as.factor(byear),
                   subset =
                     nkey_male %in% 2:5 &
                     south_socstate == TRUE &
                     sex_score_std_strong < 1.3 & 
...
m.bni.north = felm(death_age ~ bni_root_master | key + birth_order + as.factor(byear),
                   subset =
                     nkey_male %in% 2:5 &
