...
The black name index (BNI) is a summary measure of how distinctively Black first names are: 
...
Controlling for family age 12-16. 
...
## restrict to two or more siblings with 500+ names
sibs <- census_blacks_sibs %>% 
  drop_na(educ_yrs, bni) %>% 
  filter(name_freq > 100) %>% 
...
## 
south_socstate.vec = c(10, 11, 12, 13, 24, 37, 45, 51, 54, 1, 21, 28, 47, 05, 22, 40, 48)

## add region
sibs <- sibs %>% 
  mutate(currently_enrolled = case_when(
    SCHOOL == 2 ~ 1,
    SCHOOL == 1 ~ 0),
    south_socstate = case_when(
      STATEFIP %in% south_socstate.vec ~ 1,
      TRUE ~ 0
    ))


## run fixed effect models for southern states
bni.south <- felm(educ_yrs ~ bni + nick_mpc | as.factor(AGE),
     data = sibs,
     subset = south_socstate == TRUE)

bni.south.sibfe <- felm(educ_yrs ~ bni + nick_mpc | as.factor(SERIALP) + as.factor(AGE),
     data = sibs,
     subset = south_socstate == TRUE)

## run fixed effect models
bni.nonsouth <- felm(educ_yrs ~ bni + nick_mpc | as.factor(AGE),
     data = sibs,
     subset = south_socstate == FALSE)

bni.nonsouth.sibfe <- felm(educ_yrs ~ bni + nick_mpc | as.factor(SERIALP) + as.factor(AGE),
     data = sibs,
     subset = south_socstate == FALSE)

## print our fixed effect models
stargazer::stargazer(bni.nonsouth,
                     bni.nonsouth.sibfe, 
                     bni.south, 
                     bni.south.sibfe,
                     object.names = TRUE,
...
Is BNI associated with a higher / lower educational attainment?
...
Is BNI associated with still being in school? 

```{r, eval = F}

## run fixed effect models for southern states
bni.south <- felm(currently_enrolled ~ bni + nick_mpc | as.factor(AGE),
     data = sibs,
     subset = south_socstate == TRUE)

bni.south.sibfe <- felm(currently_enrolled ~ bni + nick_mpc | as.factor(SERIALP) + as.factor(AGE),
     data = sibs,
     subset = south_socstate == TRUE)

## run fixed effect models
bni.nonsouth <- felm(currently_enrolled ~ bni + nick_mpc | as.factor(AGE),
     data = sibs,
     subset = south_socstate == FALSE)

bni.nonsouth.sibfe <- felm(currently_enrolled ~ bni + nick_mpc | as.factor(SERIALP) + as.factor(AGE),
     data = sibs,
     subset = south_socstate == FALSE)
...
#                                                  currently_enrolled                              
#                            (1)                (2)                 (3)                 (4)        
#                        bni.nonsouth    bni.nonsouth.sibfe      bni.south        bni.south.sibfe  
# -------------------------------------------------------------------------------------------------
# bni                       -0.010             0.006             -0.060***            -0.002       
#                          (0.006)            (0.008)             (0.005)             (0.005)      
#                                                                                                  
# nick_mpc                -0.023***            0.004             -0.031***            -0.0002      
#                          (0.003)            (0.004)             (0.002)             (0.002)      
#                                                                                                  
# -------------------------------------------------------------------------------------------------
# Observations              32,793             32,793             192,984             192,984      
# R2                        0.047              0.730               0.100               0.859       
# Adjusted R2               0.047              0.493               0.100               0.741       
# Residual Std. Error 0.228 (df = 32786) 0.167 (df = 17453) 0.403 (df = 192977) 0.216 (df = 105449)
# =================================================================================================
# Note:                                                                 *p<0.1; **p<0.05; ***p<0.01
